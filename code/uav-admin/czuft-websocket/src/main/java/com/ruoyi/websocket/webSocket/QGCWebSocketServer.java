...
@Slf4j
@ServerEndpoint("/ws")
public class QGCWebSocketServer {
    ...
    public static CopyOnWriteArraySet<QGCWebSocketServer> webSocketSet = new CopyOnWriteArraySet<>();
    ...
    public void onError(Session session, Throwable throwable) {
        log.error("QGC 连接发生报错 " + throwable.toString());
        synchronized (this) {
    ...
    public void onClose(Session session) {
        ...
        // 使用线程安全的Map，无需额外同步
        //sessionPool.entrySet().removeIf(entry -> entry.getValue().equals(session));
        log.info("QGC 连接断开,当前在线设备数为：{}", webSocketSet.size());
    }
    ...
    public void onMessage(String message, Session session) {
        ...
        msgService.QGCHandleMessage(message);
    }
