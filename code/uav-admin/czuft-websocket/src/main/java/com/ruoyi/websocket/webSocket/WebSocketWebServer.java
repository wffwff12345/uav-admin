...
@Component
@Slf4j
@ServerEndpoint("/websocket/web/{macId}")
public class WebSocketWebServer {
    ...
    public static CopyOnWriteArraySet<WebSocketWebServer> webSocketSet = new CopyOnWriteArraySet<>();
     ...
     */
    @OnOpen
    public void onOpen(Session session, @PathParam("macId") Integer macId) {
        log.info("Web WebSocket建立连接中,连接设备macId：{}", macId);
        try {
    ...
    public void onError(Session session, Throwable throwable) {
        log.error("Web WebSocket连接发生报错" + throwable.toString());
        synchronized (this) {
    ...
    public void onClose() {
        ...
        log.info("Web WebSocket连接断开,当前在线设备数为：{}", webSocketSet.size());
    }
     ...
     * @param message 发送的消息
     */
    public static void sendMessageByMac(int macId, String message) {
        try {
            Session session = sessionPool.get(macId);
            if (Objects.nonNull(session)) {
                log.info("设备ID：" + macId + ",Web 推送内容：" + message);
