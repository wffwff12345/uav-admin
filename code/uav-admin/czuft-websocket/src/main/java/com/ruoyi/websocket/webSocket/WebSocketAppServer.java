...
@Component
@Slf4j
@ServerEndpoint("/websocket/app/{sysId}")
public class WebSocketAppServer {

    public static IWebSocketAppMsgService msgService;

    @Autowired
    public void setMsgService(IWebSocketAppMsgService msgService) {
    ...
    public static CopyOnWriteArraySet<WebSocketAppServer> webSocketSet = new CopyOnWriteArraySet<>();
     ...
     */
    @OnOpen
    public void onOpen(Session session, @PathParam("sysId") String sysId) {
        log.info("App WebSocket建立连接中,连接设备sysId：{}", sysId);
        try {
            Session historySession = sessionPool.get(Integer.valueOf(sysId));
            // historySession不为空,说明已经有设备连接,应该删除连接的WebSocket对象
            if (Objects.nonNull(historySession)) {
                log.info("删除登陆的WebSocket对象", sysId);
                //webSocketSet.remove(historySession);
                sessionPool.remove(Integer.valueOf(sysId));
        ...
        sessionPool.put(Integer.valueOf(sysId), session);
    ...
    public void onMessage(String message, Session session) {
        ...
        System.out.println("收到来自设备App" + this.sysId + "的消息，设备地址：" + session.getMessageHandlers() + "，消息内容：" + message);
        msgService.handleMessage(this.sysId, message);
    }
     ...
     * @param message 发送的消息
     */
    public static void sendMessageByMac(int sysId, String message) {
        try {
            Session session = sessionPool.get(sysId);
            if (Objects.nonNull(session)) {
                log.info("设备ID：" + sysId + ",App 推送内容：" + message);
